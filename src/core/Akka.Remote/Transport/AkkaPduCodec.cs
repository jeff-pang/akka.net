...
namespace Akka.Remote.Transport
{
    ...
    internal class AkkaPduProtobuffCodec : AkkaPduCodec
    {
        ...
        {
            var ackBuilder = new AcknowledgementInfo {
                CumulativeAck = (ulong)ack.CumulativeAck.RawValue
            };

            return ack.Nacks.Aggregate(ackBuilder, (current, nack) => {
                current.Nacks.Add((ulong)nack.RawValue);
                return current;
            });
        ...
        {
            var ackAndEnvelopeBuilder = new AckAndEnvelopeContainer();
            var envelopeBuilder = new RemoteEnvelope
            {
                Recipient = SerializeActorRef(recipient.Path.Address, recipient)
            };

            if (senderOption != null && senderOption.Path != null) {
                envelopeBuilder.Sender = SerializeActorRef(localAddress, senderOption);                
            }
            if (seqOption != null) {
                envelopeBuilder.Seq = (ulong)seqOption.RawValue;
            }
            if (ackOption != null)
            {
                ackAndEnvelopeBuilder.Ack = AckBuilder(ackOption);
            }

            envelopeBuilder.Message =serializedMessage;
            ackAndEnvelopeBuilder.Envelope = envelopeBuilder;

            return ackAndEnvelopeBuilder.ToByteString();
        }
...
#region Internal methods
        private IAkkaPdu DecodeControlPdu(AkkaControlMessage controlPdu)
        {
            switch (controlPdu.CommandType)
            {
                case CommandType.Associate:
                    if (controlPdu.HandshakeInfo !=null)
                    {
        ...
        {
            var controlMessageBuilder = new AkkaControlMessage
            {
                CommandType = code
            };
            ...
            {
                controlMessageBuilder.HandshakeInfo = handshakeInfo;
            }

            return new AkkaProtocolMessage {
                Instruction = controlMessageBuilder
            }.ToByteString();
        }
