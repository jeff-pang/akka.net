...
namespace Akka.Remote.Serialization
{
    ...
    public class MessageContainerSerializer : Serializer
    {
        ...
        private byte[] SerializeSelection(ActorSelectionMessage sel)
        {
            ...
            var selectionEnv = new SelectionEnvelope();
            var message = sel.Message;
            Serializer serializer = system.Serialization.FindSerializerFor(message);
            selectionEnv.EnclosedMessage = ByteString.CopyFrom(serializer.ToBinary(message));
            selectionEnv.SerializerId=serializer.Identifier;
            ...
            if (serializer2 != null)
            {
                ...
                {
                    selectionEnv.MessageManifest = ByteString.CopyFromUtf8(manifest);
                }
            }
            else
            {
                if (serializer.IncludeManifest)
                    selectionEnv.MessageManifest = ByteString.CopyFromUtf8(message.GetType().AssemblyQualifiedName);
            }

            foreach (SelectionPathElement element in sel.Elements)
            {
                element.Match()
                    .With<SelectChildName>(m => selectionEnv.Pattern.Add(BuildPattern(m.Name, PatternType.ChildName)))
                    .With<SelectChildPattern>(
                        m => selectionEnv.Pattern.Add(BuildPattern(m.PatternStr, PatternType.ChildName)))
                    .With<SelectParent>(m => selectionEnv.Pattern.Add(BuildPattern(null, PatternType.Parent)));
            }

            return selectionEnv.ToByteArray();
        }
        ...
        public override object FromBinary(byte[] bytes, Type type)
        {
            ...
            SelectionPathElement[] elements = selectionEnvelope.Pattern.Select<Selection, SelectionPathElement>(p =>
            {
